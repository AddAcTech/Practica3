  ...
  useEffect(() => {
    const fetchUser = async () => {
      try {
        ...
        // Llamar al endpoint para obtener el perfil
        const response = await fetch(`http://192.168.1.81:3000/user/profile?email=${parsedUser.email}`);
        const data = await response.json();

        if (response.ok) {
          setUser(data.user); // Actualizar el estado con los datos del perfil
        } else {
